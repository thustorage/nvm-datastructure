...
extern char* backend_thread_start_addr;
#endif
...
#ifdef USE_DAX
    backend->vel.thread.start_addr = backend_thread_start_addr;
    backend->vel.thread.affinityNodeID = -2;
    log_debug(LOG_DEBUG, "backend thread's start_addr is %p", backend->vel.thread.start_addr);
#endif
    return VR_OK;
}
...
backends_run(void)
{
    ...
    //log_debug(LOG_DEBUG, "enter backends_run, start %d threads", thread_count);
