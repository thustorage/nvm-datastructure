...
#ifndef FPTREE_MAP_H_HEADER
#define FPTREE_MAP_H_HEADER
...
public:
    ...
    inline std::pair<iterator, bool> update(const value_type& x)
    {
    ...
    /// Bulk load a sorted range [first,last). Loads items into leaves and constructs
    /// a B-tree above them. The tree must be empty when calling this function.
