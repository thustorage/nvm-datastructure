...
extern char* master_thread_start_addr;
#endif
...
#ifdef USE_DAX
    master.vel.thread.start_addr = master_thread_start_addr;
    master.vel.thread.affinityNodeID = -1;
    log_debug(LOG_DEBUG, "master thread's start_addr is %p", master.vel.thread.start_addr);
#endif
...
master_run(void)
{
    //log_debug(LOG_DEBUG, "enter master_run");
