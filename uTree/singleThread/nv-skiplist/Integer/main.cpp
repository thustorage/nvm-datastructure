...
int main()
{
    ...
    void *pmem = mmap(NULL, (uint64_t)600ULL * 1024ULL * 1024ULL * 1024ULL, PROT_READ | PROT_WRITE,
                        MAP_SHARED, fd, 0);
    ...
    gettimeofday(&start_time, NULL);
    uint64_t valueRec;
    //ProfilerStart("skiplist_search");
    for (int i = 0; i < OP_NUM; i++) {
        search(l, keys[i], &valueRec);
    }
    //ProfilerStop();
    gettimeofday(&end_time, NULL);
    time_interval = 1000000 * (end_time.tv_sec - start_time.tv_sec) + end_time.tv_usec - start_time.tv_usec;
