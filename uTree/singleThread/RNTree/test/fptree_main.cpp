...
int main() {
    ...
    Index<uint64_t, void *, 64> *bt = nvindex::getIndex(FP_TREE);
    ...
    gettimeofday(&start_time, NULL);
    // ProfilerStart("fptree_search");
    for (int i = 0; i < OP_NUM; i++) {
        bt->get(keys[i], latency_breaks);
    }
    // ProfilerStop();
    gettimeofday(&end_time, NULL);
    time_interval = 1000000 * (end_time.tv_sec - start_time.tv_sec) +
    ...
    printf("\n*********************************** The update operations "
            "********************************\n");
    gettimeofday(&start_time, NULL);
    // ProfilerStart("fptree_update");
    for (int i = 0; i < OP_NUM; i = (i + 1) % OP_NUM) {
      // simulate the 64B-value-persist latency.
      // emulate_latency_ns(EXTRA_SCM_LATENCY);
      bt->update(keys[i], (void *)i, latency_breaks);
    }
    // ProfilerStop();
    gettimeofday(&end_time, NULL);
    time_interval = 1000000 * (end_time.tv_sec - start_time.tv_sec) +
