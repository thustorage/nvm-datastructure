...
//#define STATICS_MALLOC
    ...
    uint64_t leaf_sum    = 0, inner_sum = 0, index_sum   = 0, kv_sum    = 0;
...
void clflush(char *addr, int len)
{
    ...
    wear_byte_sum += len;
#endif
    for (uintptr_t uptr = (uintptr_t)addr & ~(FLUSH_ALIGN - 1); uptr < (uintptr_t)addr + len; uptr += FLUSH_ALIGN) {
...
#ifdef STATICS_EVALUATION
              latency_end = asm_rdtsc();
              sort_latency += CYCLE2NS(latency_end - latency_start);
              sort_flush += flush_sum - flush_start;
              sort_wear += wear_byte_sum - wear_byte_start;
#endif
#ifdef STATICS_EVALUATION
              latency_start = asm_rdtsc();
              flush_start = flush_sum;
