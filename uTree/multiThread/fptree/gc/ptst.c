...
ptst_t *critical_enter(void)
{
    ...
    if ( ptst == NULL ) 
    {
        for ( ptst = ptst_first(); ptst != NULL; ptst = ptst_next(ptst) )  //获取可用的线程状态
        {
        ...
        if ( ptst == NULL ) 
        {
            ptst = ALIGNED_ALLOC(sizeof(*ptst));  //创建线程的状态
            if ( ptst == NULL ) exit(1); 
            memset(ptst, 0, sizeof(*ptst));
            ptst->gc = gc_init();                 //初始化gc
            rand_init(ptst);                      //初始化随机数
            ptst->count = 1;                      //count?
            id = next_id;
            while ( (oid = CASIO(&next_id, id, id+1)) != id ) id = oid;
            ptst->id = id;
            new_next = ptst_list;                 //加入到线程状态的链表
            do {
        ...
        pthread_setspecific(ptst_key, ptst);      //绑定到线程特殊的变量上
    }
...
void _init_ptst_subsystem(void) 
{
    ...
    next_id   = 0;
    WMB();
    // 创建一个同名的ptst_key，不同线程都可以看到，但可以存储不同的线程私有值
